...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        if self.individual:
            ...
            din = self.dominance_freq
            dout= int(self.dominance_freq*self.length_ratio)
            if configs.VE_len == 0:
                self.freq_upsampler = nn.Linear(din, dout).to(torch.cfloat) # complex layer for frequency upcampling]
            else:
                self.freq_upsampler = VarEmbed(configs.enc_in, configs.VE_len, din, dout, configs.ParamRatio, 1, 1, configs.usePreprocess) 
